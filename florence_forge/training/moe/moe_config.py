...
class MoEConfig(BaseModel):
    ...
    z_loss_weight: float = Field(
        default=0.001, ge=0.0, description="路由器 z-loss 权重"
    )
    capacity_factor: float = Field(default=1.25, ge=1.0, description="专家容量溢出因子")
    enable_expert_parallelism: bool = Field(
        default=False, description="是否启用专家并行"
    )
    device_map: str = Field(default="auto", description="设备映射策略")
    hard_routing: bool = Field(
        default=False, description="是否使用硬路由（每个 token 只路由到 1 个专家）"
    )
