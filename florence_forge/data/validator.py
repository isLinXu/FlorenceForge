...
class DataValidator:
    ...
    def _validate_sample_list(self, samples: List[Dict], base_path: Path) -> Dict[str, Any]:
        """验证样本列表"""
        if not samples:
                        ...
                        try:
                            with Image.open(full_image_path) as img:
                                ...
                                if img.size[0] < 32 or img.size[1] < 32:
                                    self._add_warning(f"样本{i}: 图像尺寸过小 {img.size}")
    ...
    def _validate_sample_fields(self, sample: Dict[str, Any], index: int) -> bool:
        ...
        optional_fields = ['task_type', 'metadata']
        ...
        for j, conv in enumerate(conversations):
            ...
            if 'from' not in conv or 'value' not in conv:
                self._add_error(f"样本{index}: conversations[{j}]缺少'from'或'value'字段")
                return False
            
            if conv['from'] not in ['human', 'gpt']:
                self._add_warning(f"样本{index}: conversations[{j}]['from']值异常: {conv['from']}")
        
        return True
    
    def _validate_task_specific_content(self, sample: Dict[str, Any], index: int) -> None:
        """验证任务特定内容"""
...
def validate_data_format(data_path: Union[str, Path], strict_mode: bool = False) -> Dict[str, Any]:
    """验证数据格式的便捷函数
