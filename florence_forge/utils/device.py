...
def get_optimal_device(
    ...
    if prefer_gpu:
        ...
        if torch.cuda.is_available():
            ...
            cuda_devices = get_cuda_info()
            suitable_devices = [
                dev for dev in cuda_devices 
                if dev.memory_available >= min_memory_gb
            ]
...
def set_device(
    ...
    if verbose:
        ...
        if device.type == "cuda":
            ...
            print(f"使用设备: {cuda_info}")
        elif device.type == "mps":
            mps_info = device_info["mps"]
            print(f"使用设备: {mps_info}")
        else:
            cpu_info = device_info["cpu"]
            print(f"使用设备: {cpu_info}")
...
def move_to_device(
    ...
    if isinstance(obj, torch.Tensor):
        ...
        moved_items = [
            move_to_device(item, device, non_blocking)
            for item in obj
        ]
...
def check_device_compatibility(
    ...
    if cpu_info.memory_available >= required_memory_gb:
        ...
        result["warnings"].append(f"CPU内存不足: {cpu_info.memory_available:.1f}GB < {required_memory_gb}GB")
        ...
        if self.device.type == "cuda":
            ...
            if cuda_devices:
                ...
                if device_id < len(cuda_devices):
                    ...
                    summary.append(f"内存: {cuda_info.memory_available:.1f}GB/{cuda_info.memory_total:.1f}GB")
            ...
            summary.append(f"内存: {cpu_info.memory_available:.1f}GB/{cpu_info.memory_total:.1f}GB")
