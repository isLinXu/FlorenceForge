...
def test_select_trainer_class_returns_canonical_trainer():
    from florence_forge.training.trainer import MultiTaskTrainer

    assert _select_trainer_class(None) is MultiTaskTrainer
    assert _select_trainer_class("v2") is MultiTaskTrainer


def test_select_trainer_class_rejects_v1():
    import pytest

    with pytest.raises(ValueError, match="v1 训练栈已在 v2.0.0 移除"):
